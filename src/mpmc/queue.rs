...
impl<T: Sized + 'static> Queue<T> {
    ...
    pub fn push(&self, value: T) {
        ...
        if old_node != ptr::null_mut() {
            unsafe { (*old_node).next.store(node, Ordering::SeqCst); }
        }
        // 4. if the tail is not yet pointing anywhere set the tail to the node just inserted
        dmb();
        self.tail.compare_and_swap(ptr::null_mut(), node, Ordering::AcqRel);
        dsb();
    }
    ...
    pub fn pop(&self) -> Pop<T> {
        ...
        self.head.compare_and_swap(node, ptr::null_mut(), Ordering::AcqRel);
        dsb();
        ...
        if next_node != ptr::null_mut() {
            self.tail.compare_and_swap(ptr::null_mut(), next_node, Ordering::AcqRel);
